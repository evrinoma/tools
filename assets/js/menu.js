...
(function ($) {
    $.widget("ui.menubar", {
        _create: function () {
            var self = this;
            var $button = self.element.children("button, a");;
            $button.addClass("ui-state-default").addClass("ui-button-text-only");
            $button.next("ul").each(function (i, elm) {
                $(elm).flyoutmenu({
                    select: self.options.select,
                    input: $(elm).prev()
                }).hide().addClass("ui-menu-flyout").addClass("ui-button-text-icon-secondary").removeClass("ui-button-text-only");
            });
            $button.each(function (i, elm) {
                $(elm).click(function (event) {
                    $(document).find(".ui-menu-flyout").hide();
                    if ($(this).next().is("ul")) {
                        $(this).next().data("uiFlyoutmenu").show();
                        $(this).next().css({
                            position: "absolute",
                            top: 0,
                            left: 0
                    ...
                    icons: {secondary: ($(elm).next("ul").length > 0) ? 'ui-icon-triangle-1-s ui-button-icon-secondary' : ''}
                });
                var text = $(elm).text().trim();
                if ($(elm).next("ul").length > 0) {
                    $(elm).html($(elm).html().replace(text, ""));
                    $(elm).find("span.ui-button-icon-space").text(text).addClass("ui-button-text")
                } else {
                    var $span = $('<span>').addClass("ui-button-text").text(text);
                    $span.appendTo($(elm).text(""));
                }
            });
        }
    });
